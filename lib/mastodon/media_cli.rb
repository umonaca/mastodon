...
module Mastodon
  class MediaCLI < Thor
    ...
    def remove
      ...
      processed, aggregate = parallelize_with_progress(MediaAttachment.cached.where.not(remote_url: '').where.not('remote_url LIKE ?', '%smuglo%').where('created_at < ?', time_ago)) do |media_attachment|
